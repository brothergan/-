  			...
  			<table id="myDocument_table" class="table table-striped table-bordered table-hover">
		   		...
		   		<tfoot id="template" style="display:none">
		   			<tr>
		   				<td>{adminId}</td>
		   				<td>{lastLoginTime}</td>
		   				<td><a href="javascript:loadAdminPrivileges({adminId})"  class="btn btn-warning btn-xs">设置权限</a></td>
		   			</tr>
		   		</tfoot>
		   	</table>
		   	<div>
...
<script>
	function loadAdminPrivileges(_id){
		$.ajax({
			...
			dataType:"json",
			success:function(data){
				location.href="setAdminPrivileges.jsp";
			}
		});
	}
</script>