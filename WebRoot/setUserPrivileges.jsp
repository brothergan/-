  				...
  				<c:if test="${not empty menuList }">
				    <ul>
				    	<c:forEach items="${menuList }" var="m1">
				   			<c:if test="${m1.menuId>=2&&m1.menuId<=6 }">
				   				<c:if test="${m1.parentMenu.menuId==1 }">
				   					<li id="li_${m1.menuId }"><input type="checkbox" value="menu_${m1.menuId }" id="cb_${m1.menuId }" onclick="doChecked('li_${m1.menuId}',this.checked)" name="resourceIds" ${m1.menuStatus==1?"checked='checked'":"" } />${m1.menuName }
				   						<c:forEach items="${menuList }" var="m2">
				   							<c:if test="${m2.menuId>=2&&m2.menuId<=6&&m2.parentMenu.menuId==m1.menuId }">
				   								<ul>
				   						...
				   						<c:if test="${not empty buttonList }">
					  						<c:forEach items="${buttonList }" var="b">
					  							<c:if test="${b.menu.menuId==m1.menuId }">
					  								<li id="li_${m1.menuId }_${b.buttonId }"><input type="checkbox" value="button_${b.buttonId }" id="cb_${m1.menuId }_${b.buttonId }" onclick="doChecked('li_${m1.menuId }_${b.buttonId }',this.checked)" name="resourceIds" ${b.buttonStatus==1?"checked='checked'":"" } />${b.buttonName }</li>
					  							</c:if>
					  						</c:forEach>
						  				</c:if>
						  				</ul>
