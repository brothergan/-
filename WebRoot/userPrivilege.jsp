					...
					success:function(data){
						...
						if(data.list && data.list.length>0){
							...
							$.each(data.list,function(i,ele){
								var _tr=templateTr;
								_tr=_tr.replace(/{userId}/g,ele.userId);
								_tr=_tr.replace(/{userLoginName}/g,ele.userLoginName);
								_tr=_tr.replace(/{lastLoginTime}/g,ele.lastLoginTime);
								html.push(_tr);
							});
						}else{
							html.push("<tr><td colspan='4'>没有找到数据</td></tr>");
						}
  			...
  			<table id="myDocument_table" class="table table-striped table-bordered table-hover">
		   		...
		   		<tfoot id="template" style="display:none">
		   			<tr>
		   				...
		   				<td>{userLoginName}</td>
		   				<td>{lastLoginTime}</td>
		   				<td><a href="javascript:loadUserPrivileges({userId})"  class="btn btn-warning btn-xs">设置权限</a></td>
		   			</tr>
		   		</tfoot>
		   	</table>
		   	<div>
...
<script>
	function loadUserPrivileges(_id){
		$.ajax({
			url:'loadPrivileges.action',
			data:{
				roleType:"user",
				roleId:_id
			},
			...
			dataType:"json",
			success:function(data){
				location.href="setUserPrivileges.jsp";
			}
		});
	}
</script>