					...
					success:function(data){
						...
						if(data.list && data.list.length>0){
							...
							$.each(data.list,function(i,ele){
								var _tr=templateTr;
								_tr=_tr.replace(/{informId}/g,ele.informId);
								_tr=_tr.replace(/{informReason}/g,ele.informReason);
								_tr=_tr.replace(/{informDate}/g,ele.informDate);
								html.push(_tr);
							});
						}else{
  			...
  			<table id="myDocument_table" class="table table-striped table-bordered table-hover">
		   		...
		   		<tfoot id="template" style="display:none">
		   			<tr>
		   				<td>{informId}</td>
		   				<td>
		   				<a href="javascript:informInfo({informId})">
		   					{informReason}
		   				</a>
		   				</td>
		   				<td>{informDate}</td>
...
<script>
	function informInfo(_id){
		$.ajax({
			url:'loadInform.action',
			data:{informId:_id},
			type:"post",
			dataType:"json",
			success:function(data){
				/* location.href="informInfo.jsp"; */
				layer_show("举报信息","informInfo.jsp",'410','510');
			}
		});
	}
</script>